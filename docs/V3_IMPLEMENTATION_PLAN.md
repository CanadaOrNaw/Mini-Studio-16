...
| PO-33 tone/filter/trim, per-slice edit and copy | Parameters/locks plus independent sound metadata copy and asynchronous PCM-rebuilding cross-slot slice copy (same-rate assets; mismatches reject explicitly) | Copy/quota/seqlock/project tests and firmware worker compile |
...
| MEDO Drum/Bass/Chord/Lead/Sample roles | Five event tracks assigned to the documented roles, with dynamic role gain on replay rather than baking the mixer level into events | Role/volume/octave/clear/overdub tests |
...
| MEDO scales/transposition/arpeggio | Natural/major/minor pentatonic mapping, role octave, and an actual bounded chord arpeggiator with enable, four directions and four rates | Note/scale/direction/rate/output tests |
...
| M12 three-in-one workflows | Behavior-level HiChord modes/fixed loops/practice/ear rounds, real PO slice PCM copy/swing/punch effects, and MEDO roles/dynamic mix/real arp/quantize/scales/shared length/gestures, all reachable from UI/serial and persisted in GBX v9 | Pure timing/rate/arp/state/project/protocol/CLI tests, worker syntax checks, full legacy suite and malformed-input fuzz | Performance feel, motion/pitch thresholds, codec full duplex, and end-to-end audio timing |
