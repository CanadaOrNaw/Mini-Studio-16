...
| --- | --- | --- |
| HiChord instrument | Seven diatonic chord keys; 10 scales, 28 chord types, three directional maps, inversions, locks, root/slash bass and voice leading; all 15 modes with three strum speeds, six arp patterns/three layers/nine rates, repeat, monophonic Lead and latched Drone; six programmed kits plus a user slot/56 grooves; directional auto-drum; 4/8/12/16-step sequencer and three auto-bounce sources; timed Chord Hiro, audible one/four-chord Ear Trainer, pitch-detected mic sample, tuner, mixer, presets, effects and vocoder | Playing feel, pitch-detection calibration, and full-duplex mic vocoder on the physical codec |
...
| Motion | BMI270 filtering with five mappable sources (tilt X/Y, acceleration, gyro, shake, slap), four internal mappings, synth cutoff/resonance targets, MIDI CC and recordable automation; MEDO additionally emits eight Click/Press/Slide/Slap/Tilt/Shake/Wiggle/Move control messages, of which Slap/Tilt/Shake/Wiggle/Move originate in the motion layer | IMU calibration and gesture thresholds |
...
| VOCODER | Eight-band vocoder; Loop 1 is the software-complete modulator and Audio Cap line is implemented but needs the cap to validate. The onboard-mic modulator requires simultaneous mic capture and speaker output, which is a hardware gate (see full-duplex, section 11 of the test guide) — selecting a source that cannot supply audio leaves the dry bus untouched rather than muting it |
...
| KO | Pattern keys `4`–`-` are momentary PO punch effects (A/B selects effects 1-8/9-15; the sixteenth slot is dry); WRITE/PLAY records them; `n` changes swing; `z` clears the step lock |
